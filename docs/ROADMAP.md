...
- [x] KE-QR parsing: recognize till, paybill, and send-money QR formats (2026-07-24) — recognizes the real-world PesaQR pipe-delimited format (BG/PB/SM prefixes, confirmed from a public open-source reference) plus a best-effort EMVCo TLV fallback; Detected-automatically sheet now shows Till/Paybill specifically. 26 parser tests.
...
- [ ] Payout engine: rate quoting, sats debit, idempotency — vendor-agnostic `PayoutProvider` interface + sandbox mock implementation built (2026-07-24, 16 tests), not yet wired to the Scan screen or any real vendor — scanning a KE-QR still correctly shows "coming soon" until a vendor is chosen
- [x] Resolve Splice Africa vs. IntaSend vs. direct Daraja — **decided 2026-07-24: direct Safaricom Daraja API.** Registration steps and technical shape documented in `docs/DARAJA_INTEGRATION.md`. Founder to begin the M-Pesa for Business + Daraja production application (realistic timeline 2-4 weeks); sandbox development starts immediately in parallel via a free developer.safaricom.co.ke account.
- [ ] Confirm the Splice/Tando ownership question — no longer blocking now that Daraja is the chosen vendor; left unconfirmed, low priority unless Splice comes back into consideration later
